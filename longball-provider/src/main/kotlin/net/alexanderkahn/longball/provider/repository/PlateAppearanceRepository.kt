...
interface PlateAppearanceRepository: LongballRepository<PlateAppearanceEntity> {
    fun findFirstBySideAndOwnerOrderByIdDesc(side: InningSideEntity, owner: UserEntity): PlateAppearanceEntity?
    fun findBySideAndOwner(side: InningSideEntity, owner: UserEntity): List<PlateAppearanceEntity> //TODO should this be deleted?

    @Query("select p from plate_appearance p where p.owner = ?2 and p.side = ?5 and p.side.side = ?4 and p.side.inning.game = ?3")
    fun findByOwnerAndGameAndInningNumberAndSide(pageable: Pageable, owner: UserEntity, gameId: UUID, inningNumber: Int, side: Side): Page<PlateAppearanceEntity>
}